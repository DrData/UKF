...
int main()
{
	...
	else
	{
		...
		h.onMessage([&USKFusion, &tools, &estimations, &ground_truth](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
			...
			if (length && length > 2 && data[0] == '4' && data[1] == '2')
			{
				...
				if (s != "") {
					...
					if (event == "telemetry") {
						...
						USKFusion.ProcessMeasurement(meas_package);

						//Push the current estimated x,y positon from the Kalman filter's state vector

						VectorXd estimate(4);

						double p_x = USKFusion.theState_.x_(0);
						double p_y = USKFusion.theState_.x_(1);
						double v = USKFusion.theState_.x_(2);
						double yaw = USKFusion.theState_.x_(3);
...
void ProcessToFile()
{
	...
	std::string foutput = "D:\\Google\\GDrive443\\SDCT2\\UKF\\rsme_set1.txt";
	...
	if (sensorDataFile.is_open()) {
		while (std::getline(sensorDataFile, sMeasure))
		{
			...
			double p_y = USKFusion.theState_.x_(1);
			double v = USKFusion.theState_.x_(2);
			double phi = USKFusion.theState_.x_(3);
			double v1 = v * cos(phi);
			double v2 = v * sin(phi);
	...
	sOutput << "P" << endl;
	sOutput << USKFusion.theState_.P_ << endl;

	//SelfAdjointEigenSolver<MatrixXd> eigensolver(theState.P);
	//sOutput << "Eigenvalues of P" << endl;
	//complex<double> E;
	//for (int i = 0; i < theState.P.rows(); ++i) {
